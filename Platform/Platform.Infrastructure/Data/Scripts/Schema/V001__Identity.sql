...
create table if not exists auth_organization (
    ...
    -- Hiyerarşik birim — parent zincirinden hesaplanan başlık
    title                   varchar(500),

    -- Birim türü (EXECUTIVE, INTERNAL_SYSTEM, DEPARTMENT, ADVISORY, REGION, BRANCH)
    type                    varchar(50) not null default 'DEPARTMENT' check (type in ('EXECUTIVE', 'INTERNAL_SYSTEM', 'DEPARTMENT', 'ADVISORY', 'REGION', 'BRANCH')),
    ...
    constraint fk_auth_organization_parent
        foreign key (parent_organization_id)
        references auth_organization (id),

    constraint fk_auth_organization_reports_to
...
create table if not exists auth_user (
    ...
    constraint fk_auth_user_org
        foreign key (organization_id)
        references auth_organization (id),

    constraint fk_auth_user_manager
...
create table if not exists auth_role_privilege (
    ...
    constraint fk_auth_role_privilege_role
        foreign key (role_id)
        references auth_role (id)
);
...
create table if not exists auth_user_login (
    ...
    constraint fk_auth_user_login_user
        foreign key (user_id)
        references auth_user (id)
);

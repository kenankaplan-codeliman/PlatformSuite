...
public sealed class ListBudgetCategoriesHandler : IRequestHandler<ListBudgetCategoriesQuery, Result<PagedResult<BudgetCategoryListItem>>>
{
    private readonly ICodeProDbContext _db;

    public ListBudgetCategoriesHandler(ICodeProDbContext db) => _db = db;

    public async Task<Result<PagedResult<BudgetCategoryListItem>>> Handle(ListBudgetCategoriesQuery request, CancellationToken cancellationToken)
    {
        var query = _db.BudgetCategory.AsNoTracking();
        ...
        var items = await query
            ...
            .Skip(skip)
            .Take(pageSize + 1)
            .ProjectToType<BudgetCategoryListItem>()
        ...
        {
            var parents = await _db.BudgetCategory.AsNoTracking()
        ...
        return new PagedResult<BudgetCategoryListItem>
        {
            Data = items,
